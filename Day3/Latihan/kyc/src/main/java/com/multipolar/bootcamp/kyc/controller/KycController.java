...
public class KycController {
    ...
    //get kyc by nik lewat pathvariabel/segment
    @GetMapping("/nik/{nik}")
    public ResponseEntity<Kyc> getKycByNik(@PathVariable String nik){
        Optional<Kyc> kyc = kycService.getKycByNik(nik);
        return kyc.map(ResponseEntity::ok).orElse(ResponseEntity.notFound().build());
    }

    //get kyc by firstName lewat pathvariabel/segment
    @GetMapping("/firstName/{firstName}")
    public List<Kyc> getKycByFirstName(@PathVariable String firstName){
        return kycService.getKycByFirstName(firstName);
    }

    @GetMapping("/lastName/{lastName}")
    public List<Kyc> getKycByLastName(@PathVariable String lastName){
        return  kycService.getKycByLastName(lastName);
    }
